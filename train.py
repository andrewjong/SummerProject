...
def hyperparameter_search(models, attentions, data_path, learning_rates, l2_norms, dropouts, activation_functions, vocab_dims, state_sizes, batch_sizes, num_epochs):
                ...
                for dropout in dropouts:
                    for activation_function in activation_functions:
                        for vocab_dim in vocab_dims:
                            for state_size in state sizes
                                for batch_size in batch_sizes:
                                    config = config(attention, learning_rate, l2_norm, dropout, activation_function, vocab_dim, state_size, batch_size)
                                    ...
                                    with tf.Session() as sess:
                                        ...
                                        for ep in range(int((config.num_epoch*500000)/train_size)):
                                            ...
                                            train_data = model_util.get_feed(os.path.join(data_path+  ".train"), config.batch_size, word_to_id, config.max_prem_len, config.max_hyp_len,num_iter=int(train_size/config.batch_size), shuffle = True)
