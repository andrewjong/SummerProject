...
class PIModel(object):
    ...
    def add_prediction_op(self):
        ...
        if self.model_type == "LSTMboolcomp":
            ...
            pneg2 = self.LSTMcombine(input= tf.reshape(self.embed_prems[:,4 + 11,:], [-1,300]))
            pverb2 = self.LSTMcombine(input= tf.reshape(self.embed_prems[:,5 + 11,:], [-1,300]))
            padverb2 = self.LSTMcombine(input= tf.reshape(self.embed_prems[:,6 + 11,:], [-1,300]))
            pobjectd2 = self.LSTMcombine(input= tf.reshape(self.embed_prems[:,7 + 11,:], [-1,300]))
            pobjectn2 = self.LSTMcombine(input= tf.reshape(self.embed_prems[:,8 + 11,:], [-1,300]))
            pobjecta2 = self.LSTMcombine(input= tf.reshape(self.embed_prems[:,9 + 11,:], [-1,300]))
            hsubjectd2 = self.LSTMcombine(input= tf.reshape(self.embed_hyps[:,0 + 11,:], [-1,300]))
            hsubjectn2 = self.LSTMcombine(input= tf.reshape(self.embed_hyps[:,1 + 11,:], [-1,300]))
            hsubjecta2 = self.LSTMcombine(input= tf.reshape(self.embed_hyps[:,2 + 11,:], [-1,300]))
            hneg2 = self.LSTMcombine(input= tf.reshape(self.embed_hyps[:,4 + 11,:], [-1,300]))
            hverb2 = self.LSTMcombine(input= tf.reshape(self.embed_hyps[:,5 + 11,:], [-1,300]))
            hadverb2 = self.LSTMcombine(input= tf.reshape(self.embed_hyps[:,6 + 11,:], [-1,300]))
            hobjectd2 = self.LSTMcombine(input= tf.reshape(self.embed_hyps[:,7 + 11,:], [-1,300]))
            hobjectn2 = self.LSTMcombine(input= tf.reshape(self.embed_hyps[:,8 + 11,:], [-1,300]))
            hobjecta2 = self.LSTMcombine(input= tf.reshape(self.embed_hyps[:,9 + 11,:], [-1,300]))
        ...
        if self.model_type == "sepboolcomp":
            ...
            palmostfinal2 = self.combine([psubjectd2, psubjectNP,],"comp")
            pfinal2 = self.combine([palmostfinal, pnegobjectDP2],"comp")
            ...
            halmostfinal = self.combine([hsubjectd2, hsubjectNP2,],"comp2")
            hfinal2 = self.combine([hfinal2, hnegobjectDP2],"comp2")
