...
class PIModel(object):
    ...
    def add_prediction_op(self):
        ...
        if self.model_type == "boolcomp":
            ...
            final = self.combine([subjectd, subjectNP,],"comp")
            final2 = self.combine([final, negobjectDP],"comp")
            subjectd2 = self.combine([tf.reshape(self.embed_prems[:,0 + 11,:], [-1,300]), tf.reshape(self.embed_hyps[:,0 + 11,:], [-1,300])],"lexcomp")
            subjectn2 = self.combine([tf.reshape(self.embed_prems[:,1 + 11,:], [-1,300]), tf.reshape(self.embed_hyps[:,1 + 11,:], [-1,300])],"lexcomp")
            subjecta2 = self.combine([tf.reshape(self.embed_prems[:,2 + 11,:], [-1,300]), tf.reshape(self.embed_hyps[:,2 + 11,:], [-1,300])],"lexcomp")
            neg2 = self.combine([tf.reshape(self.embed_prems[:,4 + 11,:], [-1,300]), tf.reshape(self.embed_hyps[:,4 + 11,:], [-1,300])],"lexcomp")
            verb2 = self.combine([tf.reshape(self.embed_prems[:,5 + 11,:], [-1,300]), tf.reshape(self.embed_hyps[:,5 + 11,:], [-1,300])],"lexcomp")
            adverb2 = self.combine([tf.reshape(self.embed_prems[:,6 + 11,:], [-1,300]), tf.reshape(self.embed_hyps[:,6 + 11,:], [-1,300])],"lexcomp")
            objectd2 = self.combine([tf.reshape(self.embed_prems[:,7 + 11,:], [-1,300]), tf.reshape(self.embed_hyps[:,7 + 11,:], [-1,300])],"lexcomp")
            objectn2 = self.combine([tf.reshape(self.embed_prems[:,8 + 11,:], [-1,300]), tf.reshape(self.embed_hyps[:,8 + 11,:], [-1,300])],"lexcomp")
            objecta2 = self.combine([tf.reshape(self.embed_prems[:,9 + 11,:], [-1,300]), tf.reshape(self.embed_hyps[:,9 + 11,:], [-1,300])],"lexcomp")
            ...
            #                              kernel_initializer=xavier,
            #                              use_bias=True))
            conj = self.combine([tf.reshape(self.embed_prems[:,10,:], [-1,300]), tf.reshape(self.embed_hyps[:,10,:], [-1,300])],"lexcomp")
        ...
        if self.model_type == "sepboolcomp":
            ...
            psubjectNP2 = self.combine([psubjecta2, psubjectn2],"comp",reuse=False)
            ...
            pconj = self.combine([tf.reshape(self.embed_prems[:,10,:], [-1,300]), tf.reshape(self.embed_hyps[:,10,:], [-1,300])],"lexcomp")
            ...
            hsubjectNP2 = self.combine([hsubjecta2, hsubjectn2],"comp2", reuse=False)
            ...
            hconj = self.combine([tf.reshape(self.embed_prems[:,10,:], [-1,300]), tf.reshape(self.embed_hyps[:,10,:], [-1,300])],"lexcomp")
